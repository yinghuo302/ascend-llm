...
class InferenceConfig:
    ...
    max_input_len:int = 1 # om 固定每次输入长度为max_input_len，目前设置为1
    ...
    model:str=""
    kvcache_method:str = "basic" # "basic"|"sliding-window"|'streamllm'|'H2O'
    kvcache_fixsize:bool = True
    head_len:int= 32 # 在KVCache evict时前head len会被保留
    recent_len:int = 32 # 在KVCache evict时最近recent len会被保留
    evict_len:int = 128 # KVCache 逐出的最小值，当KVCache达到最大值时将逐出evict_len个KVCache
    n_layer:int = 22
